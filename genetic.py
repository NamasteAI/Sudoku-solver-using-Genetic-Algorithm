...
Red, Green, BLUE, White, Yellow = "\033[31m", "\033[32m", "\033[94m", "\033[0m", "\033[33m"
fitness_cache = {} # Cache for fitness values to avoid recomputation
# defualt GA configurations
approach, cross_over_approach, mode = 'random', 'random', 'simple'
tournament_size, GRID_SIZE, Population_Size = 5, 4, 100
MAX_GENERATIONS, CROSSOVER_RATE, MUTATION_RATE = 1000, 0.8, 0.2
grid_counter, word_counter = 0, 0
subfolder_name, ALPHABET, LETTERS, SUBGRID_SIZE = 'grids', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', [], 2
processing_results, original_grid, generation_info = [], [], [] # list to store the results of the processing
# DataFrames for results
results_df = pd.DataFrame(columns=['Word', 'Selection Algo.', 'Crossover Algo.', 'Initial Grid', 'Solution', 
           'Word on Edge', 'Number of Solutions', 'Number Edges', 'Processing Time'])
solutions_df = pd.DataFrame(columns=['Word', 'Selection Algo.', 'Crossover Algo.', 'Initial Grid', 'Solution',
                    'Word on Edge', 'Generations', 'Processing Time', 'Parent Selection Input', 
                    'Parent Selection Output', 'Crossover Input', 'Crossover Output', 
                    'Populations Created'])
empirical_analysis = pd.DataFrame(columns=['Word', 'Selection Algorithm', 'Cross Over Algorithms', 
                                           'Execution Time', 'Number of Generations', 'Initial Grid', 
                                           'Solution'])
#################################################################   END Variables    #############################################################
...
def generate_chromosome(initial_grid, selected_word):
    ...
    all_letters = list(selected_word) # Combine existing and additional letters
...
#helper function to convert a nested list to a tuple
def to_tuple(nested):
    return tuple(to_tuple(i) if isinstance(i, list) else i for i in nested)

#simple function for caching comutations and avoiding recomputation
def compute_fitness_cached(chromosome):
    key = to_tuple(chromosome)
    if key not in fitness_cache:
        fitness_cache[key] = compute_fitness(chromosome)
...
def select_parents(population, approach='random'):
    ...
    if approach == 'wheel':
        fitnesses = [compute_fitness_cached(chromosome) for chromosome in population]
...
def generate_grid(word, num_letters=4):
    if word == '':  # Generate a random word if none is provided
        selected_word = random.choice(words) 
    else:
        selected_word = word

    # this is to make sure that the number of letters does not exced the grid size 
    num_letters = min(num_letters, GRID_SIZE**2)

    grid = [['_' for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]
    letters_to_place = random.sample(selected_word, num_letters)  # Select a subset of the word
...
def get_difficulty():
    while True:
        user_input = input("Enter the difficulty level (easy, medium, hard): ").strip().lower()
        if user_input in ['easy', 'medium', 'hard']:
            return {'easy': 4, 'medium': 3, 'hard': 2}[user_input]
        else:
            print_error("Invalid input. Please enter one of the following words: easy, medium, hard")
#################################################################  End of Helper Functions    #############################################################
...
def process_grid(grid, selected_word, selection_approach, cross_over_approach, empirical = False):
    ...
    ######### start initialization and validation of the grid #######    
    global grid_counter, results_df, original_grid, empirical_analysis, solutions_df
    generation_info, found_solutions, edge_solutions, all_populations, populations_created = [], [], [], [],[]
    _index, gen_prev, found_first_solution, word_on_edge_flag, solution_found = 0, 0, False, False, False
    solution, execution_time, start_time = None, None, time.time()
    parent_selection_input, parent_selection_output, crossover_input, crossover_output = None, None, None, None

    # if not validate_grid(grid):  
    #     print_error(f"Skipping grid {grid_counter} due to invalid grid.")
    #     return None
    original_grid = [[element for element in sublist if element != ' '] for sublist in grid]
    population = [chrm for chrm in (generate_chromosome(grid, selected_word) for _ in range(Population_Size)) 
                if not any('_' in subl for subl in chrm)]
    if not population:  # More concise check for empty population
        return None
    ######### end initialization and validation of the grid #######    

    population.sort(key=compute_fitness, reverse=True) # Sort the population based on fitness
    for generation in range(MAX_GENERATIONS): # Loop through the generations to find all possible solutions
        all_populations.append([[''.join(row) for row in individual] for individual in population])
        solution_processing_time, sol, new_row = None, None, None # Reset the solution processing time for each generation
        ...
        for individual in population:
                if compute_fitness(individual) == GRID_SIZE * 3: 
                    ...
                    sol = '\n'.join([''.join(row) for row in solution]) if solution_found else "Not Found"
                    ...
                    if individual not in found_solutions and individual not in edge_solutions:
                        if word_on_edge(individual, selected_word):
                            ...
                            solutions_df = add_solution(selected_word, selection_approach, cross_over_approach, original_grid, individual, True, 
                                                        generation - gen_prev, solution_processing_time, parent_selection_input, parent_selection_output, 
                                                        crossover_input, crossover_output, populations_created)
                            gen_prev = generation
                            print()
                        else:
                            ...
                            solution_processing_time = time.time() - solution_start_time
                            solutions_df = add_solution(selected_word, selection_approach, cross_over_approach, original_grid, individual, False, generation - gen_prev, 
                                                        solution_processing_time, parent_selection_input, parent_selection_output, crossover_input, crossover_output, 
                                                        populations_created)
                            gen_prev = generation
    ...
    print_info(f"Processed grid {grid_counter} in {processing_time:.2f} seconds.")
    grid_counter += 1
...
if __name__ == "__main__":
    ...
    if(mode == 'simple'):
        print_info('Simple mode selected, this mode will prompt the user to enter a word and then generate a grid and solve it')
        ...
        print_grid(grid)
        process_grid(grid, selected_word, 'random', 'random', False)
    elif(mode == 'empirical'):
        ...
        print()
        selected_word = get_valid_word("Enter a 4 letter word with no repated letters: ")
        num_letters = get_difficulty()
        grid, w = generate_grid(selected_word, num_letters)
        print_info(f"Generated grid for word '{selected_word}':")
        print_grid(grid)
    ...
    csv_file_path = "empirical_results_1.csv"
